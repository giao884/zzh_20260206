...
class Config:
    def __init__(self):
        parser = argparse.ArgumentParser(description="FedSelect Attack Evaluation")

        # Basic Settings
        parser.add_argument('--domain', type=str, default='fashion', choices=['digit', 'fashion', 'cifar10'], help='Dataset domain')
        parser.add_argument('--num_clients', type=int, default=10, help='Number of clients')
        parser.add_argument('--local_epochs', type=int, default=5, help='Local epochs')
        parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
        parser.add_argument('--lr', type=float, default=0.01, help='Learning rate')
        parser.add_argument('--personalization_rate', type=float, default=5.0, help='Personalization rate for FedSelect')
        ...
        parser.add_argument('--distribution_type', type=str, default='dirichlet', choices=['random', 'dirichlet'], help='Data distribution type')
        ...
        parser.add_argument('--max_classes_per_client', type=int, default=3, help='Max classes per client (random)')
...
class Client:
    def __init__(self, model_architecture, train_loader, config):
        self.model = model_architecture().to(config.device); self.train_loader = train_loader; self.config = config
    def generate_mask(self, initial_state_dict):
        self.model.load_state_dict(copy.deepcopy(initial_state_dict))
        optimizer = optim.SGD(self.model.parameters(), lr=self.config.lr, momentum=0.9); criterion = nn.CrossEntropyLoss()
        self.model.train()
        for _ in range(self.config.local_epochs):
            if not self.train_loader or len(self.train_loader.dataset) == 0: continue
            for data, target in self.train_loader:
                data, target = data.to(self.config.device), target.to(self.config.device)
                optimizer.zero_grad(); output = self.model(data); loss = criterion(output, target); loss.backward(); optimizer.step()
        final_state_dict = self.model.state_dict()
        deltas = OrderedDict(); mask = OrderedDict()
        for name in initial_state_dict.keys():
            delta = (initial_state_dict[name].cpu() - final_state_dict[name].cpu()).abs()
            deltas[name] = delta; mask[name] = torch.zeros_like(delta)
        all_deltas = torch.cat([v.flatten() for v in deltas.values() if v is not None and v.numel() > 0])
        if all_deltas.numel() == 0: return mask
        ...
        if 0 < num_params_to_personalize < all_deltas.numel():
            ...
            for name in mask: mask[name][deltas[name] >= threshold] = 1
        return mask

def generate_template_masks(config, initial_state_dict):
    print(f"Generating high-quality template masks from public {config.domain} dataset...")
    ...
    for i in tqdm(range(config.num_clients), desc="Generating Templates"):
        ...
        class_masks = [torch.cat([v.flatten() for v in Client(SimpleCNN, loader, config).generate_mask(initial_state_dict).values()]) for _ in range(config.template_runs_per_class)]
...
def run_comprehensive_attack_evaluation(config, initial_state_dict, template_masks):
    ...
    for i in tqdm(range(config.num_evaluation_trials), desc="Attack Trials"):
        ...
        client = Client(SimpleCNN, target_loader, config)
        ...
        true_top3_ordered = [cls for cls, count in true_top3_classes]

        predicted_top3_indices = np.argsort(similarities)[-3:][::-1]
        predicted_top3_set = set(predicted_top3_indices)
        predicted_top3_ordered = predicted_top3_indices.tolist()

        intersection_count = len(true_top3_set.intersection(predicted_top3_set))
        top3_recall = intersection_count / 3.0
        top3_recall_sum += top3_recall

        is_hit = (true_top3_set == predicted_top3_set)
        if is_hit:
            top3_hit_count += 1
            hit_sims = [similarities[cls] for cls in true_top3_set]
            top3_hit_similarities_sum += np.mean(hit_sims)
            is_order_match = (true_top3_ordered == predicted_top3_ordered)
            if is_order_match:
                top3_order_match_count += 1
                order_sims = [similarities[cls] for cls in true_top3_set]
                top3_order_similarities_sum += np.mean(order_sims)
        ...
        result_entry = {
            ...
            "top3_hit": int(is_hit),
            "top3_order_match": int(is_order_match if is_hit else 0),
            "true_top3": json.dumps([int(x) for x in true_top3_ordered]),
            "predicted_top3": json.dumps([int(x) for x in predicted_top3_ordered])
        }
        ...
        predicted_dominant_class = max_label; predicted_top_k_classes = set(np.argsort(similarities)[-len(true_classes_set):])
        ...
        true_sim_acc += max_similarity
        
        print(f'{i}_th Attack Target Distribution: {true_distribution}; Similarity: {similarities}; Max Similarity: {max_similarity}')
    ...
    print("\n--- Comprehensive Attack Evaluation Report (FedSelect) ---"); print(f"Total Trials: {config.num_evaluation_trials}")
    print(f"Distribution Type: {config.distribution_type.upper()}")
    ...
    avg_guessed_classes = avg_top3_recall * 3
    top3_hit_rate = top3_hit_count / config.num_evaluation_trials
    
    print(f"[5] Average Top-3 Recall (Overlap Rate): {avg_top3_recall:.4f} (Avg correct {avg_guessed_classes:.2f}/3 classes)")
    ...
    if top3_hit_count > 0:
        top3_order_match_rate = top3_order_match_count / top3_hit_count
        avg_hit_similarity = top3_hit_similarities_sum / top3_hit_count
        print(f"[7] Top-3 Match Accuracy (Order Accuracy): {top3_order_match_rate:.4f} (In {top3_hit_count} hits, {top3_order_match_count} order correct)")
        print(f"[8] Average Top-3 Similarity (when hit): {avg_hit_similarity:.4f}")
        
        if top3_order_match_count > 0:
            avg_order_similarity = top3_order_similarities_sum / top3_order_match_count
            print(f"[9] Average Top-3 Similarity (when match): {avg_order_similarity:.4f}")
        else:
            print(f"[9] Average Top-3 Similarity (when match): N/A (no order matches)")
    else:
        print(f"[7] Top-3 Match Accuracy (Order Accuracy): N/A (no hits)")
        print(f"[8] Average Top-3 Similarity (when hit): N/A (no hits)")
        print(f"[9] Average Top-3 Similarity (when match): N/A (no hits)")
