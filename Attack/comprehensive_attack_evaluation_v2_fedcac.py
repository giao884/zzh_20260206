...
class Config:
    def __init__(self):
        parser = argparse.ArgumentParser(description="FedCAC Attack Evaluation")
        ...
        parser.add_argument('--personalization_rate', type=float, default=5.0, help='Personalization rate for FedCAC')
        ...
        # Random settings for skewed distributions (only used if distribution_type='random')
        ...
        # Dirichlet settings for skewed distributions (only used if distribution_type='dirichlet')
...
class Client_FedCAC:
    ...
    def generate_mask(self, initial_state_dict):
        ...
        c_metrics = OrderedDict()
        mask = OrderedDict()
        
        for name in initial_state_dict.keys():
            final_param = final_state_dict[name]
            initial_param = initial_weights[name].to(self.config.device)
            
            g = final_param - initial_param
            v = final_param
            
            c = torch.abs(g * v)
            
            c_metrics[name] = c.cpu()
            mask[name] = torch.zeros_like(c).cpu()
            
        all_metrics = torch.cat([v.flatten() for v in c_metrics.values() if v is not None and v.numel() > 0])
        if all_metrics.numel() == 0:
            return mask
            
        num_params_to_personalize = int(all_metrics.numel() * self.config.personalization_rate / 100)
        
        if 0 < num_params_to_personalize < all_metrics.numel():
            threshold = torch.topk(all_metrics, num_params_to_personalize, largest=True).values.min()
            for name in mask:
                mask[name][c_metrics[name] >= threshold] = 1
...
def generate_template_masks(config, initial_state_dict):
    ...
    for i in tqdm(range(config.num_clients), desc="Generating Templates"):
        indices = np.where(np.array(emnist_train.targets) == i)[0]
        loader = DataLoader(Subset(emnist_train, indices), batch_size=config.batch_size, shuffle=True)
        class_masks = [torch.cat([v.flatten() for v in Client_FedCAC(SimpleCNN, loader, config).generate_mask(initial_state_dict).values()]) for _ in range(config.template_runs_per_class)]
...
def run_comprehensive_attack_evaluation(config, initial_state_dict, template_masks):
    print(f"\n--- Running {config.num_evaluation_trials} Attack Trials on Skewed Distributions (FedCAC) ---")
    ...
    top3_recall_sum = 0.0           # Sum of Top-3 Recall (for calculating average overlap)
    top3_hit_count = 0              # Top-3 perfect hit count (all 3 correct, order ignored)
    top3_order_match_count = 0      # Top-3 order perfect match count (order must also be correct)
    top3_similarities_sum = 0.0     # Sum of similarities when Top-3 set matches (for metric [9])
    top3_order_similarities_sum = 0.0  # Sum of similarities when Top-3 order matches (for metric [10])
    ...
    for i in tqdm(range(config.num_evaluation_trials), desc="Attack Trials"):
        ...
        client = Client_FedCAC(SimpleCNN, target_loader, config)
    ...
    result_df.to_csv("att_result/attack_results_fedcac.csv", index=False)
    print(f"\nAll attack results saved to: att_result/attack_results_fedcac.csv")
    ...
    print("\n--- Comprehensive Attack Evaluation Report (FedCAC) ---"); print(f"Total Trials: {config.num_evaluation_trials}")
    print(f"Distribution Type: {config.distribution_type.upper()}")
